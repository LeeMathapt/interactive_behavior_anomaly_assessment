"""
CNN-LSTM-based variational autoencoder learn predictionly.
This version fix the CNN kernel length to 3 and stride to 2.
...
class VanillaVAEPred(BaseVAE):
    ...
    def forward(self, input: torch.tensor, pred, **kwargs) -> List[torch.tensor]:
        mu, log_var = self.encode(input)
        z = self.reparameterize(mu, log_var)
        return [self.decode(z), pred, mu, log_var, z]
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        recons_loss = F.mse_loss(recons, pred)
    ...
    def predict_loss(self, *args, **kwargs):
        ...
        recons_loss = F.mse_loss(recon, pred, reduction="none")
